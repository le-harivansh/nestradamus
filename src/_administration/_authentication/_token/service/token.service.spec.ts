...
describe(TokenService.name, () => {
  ...
  beforeAll(async () => {
    ...
    jwtService.verify.mockReturnValue({ id: administrator._id.toString() });
  });
    ...
    describe.each<{ type: JwtType; configurationKeyDiscriminator: string }>([
      ...
      it('calls `ConfigurationService::getOrThrow` with the correct configuration keys', () => {
        ...
        expect(configurationService.getOrThrow).toHaveBeenCalledWith(
          `administrator.authentication.jwt.${configurationKeyDiscriminator}.duration`,
        );
        expect(configurationService.getOrThrow).toHaveBeenCalledWith(
          `administrator.authentication.jwt.${configurationKeyDiscriminator}.secret`,
        );
      });
      ...
      it('returns the configured JWT duration & secret', () => {
        expect(result).toStrictEqual({
          durationMs: configurationService.getOrThrow(
            `administrator.authentication.jwt.${configurationKeyDiscriminator}.duration` as keyof NamespacedConfiguration,
          ),
          secret: configurationService.getOrThrow(
            `administrator.authentication.jwt.${configurationKeyDiscriminator}.secret` as keyof NamespacedConfiguration,
          ),
        });
      });
    });
  ...
  describe('generateAuthenticationJwt', () => {
    ...
    describe.each<{ type: JwtType }>([
      ...
      beforeEach(() => {
        result = tokenService.generateAuthenticationJwt(type, administrator);
      });

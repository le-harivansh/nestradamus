...
import { RequiresUserRefreshToken } from './requires-user-refresh-token.guard';
...
describe(RequiresUserRefreshToken.name, () => {
  let userService: jest.Mocked<UserService>;
  let tokenService: jest.Mocked<TokenService>;
  let jwtGuard: RequiresUserRefreshToken;
  ...
  beforeAll(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        UserService,
        TokenService,
        RequiresUserRefreshToken,
      ],
    }).compile();

    userService = module.get(UserService);
    tokenService = module.get(TokenService);
    jwtGuard = module.get(RequiresUserRefreshToken);
  });

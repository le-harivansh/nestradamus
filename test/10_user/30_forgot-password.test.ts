...
import { getUserForgotPasswordOtp, registerUser } from '../helper/user';

describe(`[User] ${ForgotPasswordController.name} (e2e)`, () => {
  let start: Date;
    ...
    describe('[succeeds because]', () => {
      it('responds with HTTP:NO-CONTENT status', async () => {
        const { status } = await request(application.getHttpServer())
          ...
          .send({ destination: userData.username });
      ...
      it('receives the email with the 6-digit OTP', async () => {
        ...
        const forgotPasswordOtpRegexp = new RegExp(
          `${emailContent}\\s+${userData.username}.\\s+(\\d{${OtpService.PASSWORD_LENGTH}})`,
        );
  ...
  describe('/forgot-password/reset-password (POST)', () => {
    ...
    describe('[succeeds because]', () => {
      ...
      beforeEach(async () => {
        resetPasswordDto.otp = await getUserForgotPasswordOtp(
    ...
    describe('[fails because]', () => {
      ...
      beforeAll(async () => {
        resetPasswordOtp = await getUserForgotPasswordOtp(userData.username, {
          httpServer: application.getHttpServer(),
          mailhog,
        });
      });

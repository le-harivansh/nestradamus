...
import { getUserRegistrationOtp, registerUser } from '../helper/user';

describe(`[User] ${RegistrationController.name} (e2e)`, () => {
  let start: Date;
  ...
  describe('/register (POST)', () => {
    describe('[succeeds because]', () => {
      ...
      beforeEach(async () => {
        userData.otp = await getUserRegistrationOtp(userData.email, {
    ...
    describe('[fails because]', () => {
      ...
      beforeAll(async () => {
         ...
         * Get `new` user's OTP.
         */
        newUserData.otp = await getUserRegistrationOtp(newUserData.email, {
          httpServer: application.getHttpServer(),
          mailhog,
        });
      });
      ...
      it.each<Omit<RegistrationDto, 'otp'> & { getOtp: () => string }>([
        ...
        "responds with HTTP:BAD-REQUEST if the provided user-data is invalid [email: '$email', password: '$password']",
